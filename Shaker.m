classdef Shaker < models.muscle.AMuscleConfig
    ...
    methods
        function this = Shaker(varargin)
            %Sets default Stretch as exp
            if nargin==0
                varargin{1}='exp';
            end
            this = this@models.muscle.AMuscleConfig(varargin{2:length(varargin)});
            
            %Erlaubt es ueber den Konstruktor bereits einen Stretchverlauf
            %vorzugeben
            if(strcmp(varargin{1},'exp'))
            this.addOption('Stretch','exp');
            elseif(strcmp(varargin{1},'log'))
                this.addOption('Stretch','log');
            elseif(strcmp(varargin{1},'linear'))
                this.addOption('Stretch','linear');
            end
...
%             end
    ...
    methods(Static)
        ...
        function test_ShakerTMR
            ...
            c = models.muscle.examples.Shaker('log');
            m = c.createModel;
            c.plotTMR;
            m.plotGeometrySetup;
            

            c = models.muscle.examples.Shaker('exp');
            m = c.createModel;
            c.plotTMR;
            m.plotGeometrySetup;
    
            c = models.muscle.examples.Shaker('linear');
            m = c.createModel;
            c.plotTMR;
            m.plotGeometrySetup;
        end
